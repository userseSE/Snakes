...
template <typename... Args> struct system_arguments {
private:
...
public:
  ...
  template <typename Tuple> static auto tuple_tail_elements() {
...
template <typename F> constexpr auto get_system_arguments(F &&) {
  using arg_tuple = function_argument_types_t<F>;
  using remaining_types = std::conditional_t<
      std::is_same_v<std::remove_cvref_t<std::tuple_element_t<0, arg_tuple>>,
                     flecs::iter>,
...
template <typename F> constexpr auto get_system_arguments() {
  using arg_tuple = function_argument_types_t<F>;
  using remaining_types = std::conditional_t<
      std::is_same_v<std::remove_cvref_t<std::tuple_element_t<0, arg_tuple>>,
                     flecs::iter>,
...
template <typename F> struct IntoSystemBuilder {
...
public:
  ...
  auto build(flecs::world &world) {
    if (!builder.has_value()) {
      builder = create_system(world, args{});
    }
    return builder.value().iter(function_);
  }
  auto &builder_ref(flecs::world &world) {
...
private:
  F &&function_;
  std::optional<SystemBuilder> builder = std::nullopt;
};
